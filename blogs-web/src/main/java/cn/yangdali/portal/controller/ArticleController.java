...
public class ArticleController {
	...
	public String getArticleDetailPage(@PathVariable("articleId") Integer articleId, Model model) {
		...
		// 文章信息，分类，标签，作者，评论
		var article = articleService.getArticleByStatusAndId(ArticleStatus.PUBLISH.getValue(), articleId);
		if (article == null) {
			return "Home/Error/404";
		}

		// 用户信息
		var user = userService.getUserById(article.getArticleUserId());
		// 将关键信息置空
		...
		// 评论列表
		var commentList = commentService.listCommentByArticleId(articleId);
		model.addAttribute("commentList", commentList);

		// 相关文章
		var categoryIds = articleService.listCategoryIdByArticleId(articleId);
		var similarArticleList = articleService.listArticleByCategoryIds(categoryIds, 5);
		model.addAttribute("similarArticleList", similarArticleList);

		// 猜你喜欢
		var mostViewArticleList = articleService.listArticleByViewCount(5);
		model.addAttribute("mostViewArticleList", mostViewArticleList);

		// 获取下一篇文章
		var afterArticle = articleService.getAfterArticle(articleId);
		model.addAttribute("afterArticle", afterArticle);

		// 获取上一篇文章
		var preArticle = articleService.getPreArticle(articleId);
	...
	public Integer increaseLikeCount(@PathVariable("id") Integer id) {
		var article = articleService.getArticleByStatusAndId(ArticleStatus.PUBLISH.getValue(), id);
		var articleCount = article.getArticleLikeCount() + 1;
	...
	public Integer increaseViewCount(@PathVariable("id") Integer id) {
		var article = articleService.getArticleByStatusAndId(ArticleStatus.PUBLISH.getValue(), id);
		var articleCount = article.getArticleViewCount() + 1;
